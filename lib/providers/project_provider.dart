...
class ProjectProvider extends ChangeNotifier {
  ...
  Future<Map<String, int>> getStatistics() async {
    return _projectRepository.getStatistics();
  }

  // Tính productivity score cho project
  int productivityScore(ProjectModel project) {
    final total = project.todoCount + project.doingCount + project.doneCount;
    if (total == 0) return 0;
    return ((project.doneCount / total) * 100).round();
  }

  String productivityLabel(int score) {
    if (score <= 50) return 'Poor';
    if (score <= 75) return 'Good';
    return 'Excellent';
  }

  Color productivityColor(int score) {
