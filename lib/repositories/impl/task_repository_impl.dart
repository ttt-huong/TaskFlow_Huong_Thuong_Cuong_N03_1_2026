...
class TaskRepositoryImpl implements TaskRepository {
  ...
  /// Dùng ConnectivityService.instance.isOnline thay vì DNS lookup riêng
  /// → nhất quán, nhanh hơn (~0ms vs ~1-2s)
      ...
      if (localTask == null) {
        ...
        if (serverTask.updatedAt.isAfter(localTask.updatedAt)) {
          ...
          try {
            ...
            // Nếu đẩy server lỗi, giữ trạng thái chưa sync để retry sau
          }
        } else {
    ...
    try {
      ...
      for (var task in pendingTasks) {
        try {
          ...
          // Bỏ qua task lỗi, retry ở lần sau
        }
      }
    } catch (e) {
      // Lỗi quét danh sách → bỏ qua
    }
  }
  ...
  Future<Map<String, double>> getMemberStatistics() async {
    ...
    for (var userId in userIds) {
      ...
      stats[userId] =
          userTasks.isEmpty ? 0.0 : (done / userTasks.length) * 100;
    }
